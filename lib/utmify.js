...
/**
 * Constrói e envia o pedido "paid" a partir de uma cobrança da Velory.
 * Sandbox (live_mode === false) vai como isTest. Nunca lança.
 */
export async function notifyUtmifyPaid(charge, ip) {
  try {
    ...
    const approved = charge.paid_at ? Math.floor(Date.parse(charge.paid_at) / 1000) : Math.floor(Date.now() / 1000);
    const cents = typeof charge.amount === 'number' ? charge.amount : 7990;
    ...
    console.error('[utmify] notifyUtmifyPaid erro:', err && err.message);
